...
class SEModule(nn.Module):
    def __init__(self, channel, reduction=16):
        ...
        self.max_pool = nn.AdaptiveMaxPool2d(1)  # 添加最大池化以获取更丰富的特征
        self.fc = nn.Sequential(
            nn.Linear(channel * 2, channel // reduction, bias=False),  # 融合平均池化和最大池化的特征
    ...
    def forward(self, x):
        ...
        y = torch.cat([y_avg, y_max], 1)  # 特征拼接
        y = self.fc(y).view(b, c, 1, 1)
        return x * y.expand_as(x)


# 修改darknet_conv函数以支持SE模块
def darknet_conv_with_se(in_ch, out_ch, ksize, stride=1, dilation_rate=1, use_se=False):
    """
    Add a conv2d / batchnorm / leaky ReLU block with optional SE module.
    Args:
        in_ch (int): number of input channels of the convolution layer.
        out_ch (int): number of output channels of the convolution layer.
        ksize (int): kernel size of the convolution layer.
        stride (int): stride of the convolution layer.
        dilation_rate (int): dilation rate of the convolution layer.
    ...
    if use_se:
        stage.add_module('se', SEModule(out_ch))

    return stage


# 修改后的MultiScaleFusion类，集成SE模块
class MultiScaleFusion(nn.Module):
    def __init__(self, v_planes=[256, 512, 1024], hiden_planes=512, scaled=True, use_se=True, se_reduction=16):
        super().__init__()
        self.use_se = use_se
        ...
        self.up_modules = nn.ModuleList([
            nn.Sequential(
                ...
                darknet_conv_with_se(v_planes[-2] + hiden_planes // 2, hiden_planes // 2, ksize=1, use_se=use_se),
                darknet_conv_with_se(hiden_planes // 2, hiden_planes // 2, 3, use_se=use_se),
            ),
            nn.Sequential(
                nn.UpsamplingNearest2d(scale_factor=2) if scaled else nn.Sequential(),
                darknet_conv_with_se(v_planes[-1], hiden_planes // 2, ksize=1, use_se=use_se),
                darknet_conv_with_se(hiden_planes // 2, hiden_planes // 2, 3, use_se=use_se)
            )]
        )
        ...
        self.down_modules = nn.ModuleList([
            nn.Sequential(
                ...
                darknet_conv_with_se(hiden_planes // 2 + v_planes[0], hiden_planes // 2, ksize=1, use_se=use_se),
                darknet_conv_with_se(hiden_planes // 2, hiden_planes // 2, 3, use_se=use_se),
            ),
            nn.Sequential(
                nn.AvgPool2d(2, 2) if scaled else nn.Sequential(),
                darknet_conv_with_se(hiden_planes + v_planes[1], hiden_planes // 2, ksize=1, use_se=use_se),
                darknet_conv_with_se(hiden_planes // 2, hiden_planes // 2, 3, use_se=use_se),
            )]
        )

        # 最终投影层，也使用SE增强
        self.top_proj = darknet_conv_with_se(v_planes[-1] + hiden_planes // 2, hiden_planes, 1, use_se=use_se)
        self.mid_proj = darknet_conv_with_se(v_planes[1] + hiden_planes, hiden_planes, 1, use_se=use_se)
        self.bot_proj = darknet_conv_with_se(v_planes[0] + hiden_planes // 2, hiden_planes, 1, use_se=use_se)

    def forward(self, x):
        l, m, s = x  # l: 低层/精细特征, m: 中间层特征, s: 高层/粗糙特征
...
class EnhancedSEModule(nn.Module):
    ...
    def __init__(self, channel, reduction=16, use_spatial=True):
        super(EnhancedSEModule, self).__init__()
    ...
    def forward(self, x):
        ...
        y_avg = self.avg_pool(x).view(b, c)
        y_max = self.max_pool(x).view(b, c)
        y_channel = torch.cat([y_avg, y_max], 1)
        channel_att = self.channel_mlp(y_channel).view(b, c, 1, 1)
        x = x * channel_att
